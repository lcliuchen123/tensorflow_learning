...
# 美团重排文章核心模型代码 https://zhuanlan.zhihu.com/p/649367742
# https://github.com/Lemonace/PIER_code/blob/main/rerank_paper/pier_model/model.py
# 假定用户历史行为序列和新的序列具有相似桶，利用simhash选择相似的序列作为候选集
# 利用新的注意力层(商品和商品之间的注意力, 特征和特征之间的注意力)
# 训练时引入辅助loss（对比学习loss, 指导模型选择最优的候选序列）
...
class DNN:
    ...
    def intra_attr_self_attention(self, input):
        ...
        encoder_Q = tf.reshape(encoder_Q,
                               (tf.shape(input)[0], POI_NUM, CATE_FEATURE_EMBEDDINGS_SHAPE[1]))  # batch * 5 * 8
        encoder_K = tf.reshape(encoder_K,
                               (tf.shape(input)[0], POI_NUM, CATE_FEATURE_EMBEDDINGS_SHAPE[1]))  # batch * 5 * 8
        encoder_V = tf.reshape(encoder_V,
                               (tf.shape(input)[0], POI_NUM, CATE_FEATURE_EMBEDDINGS_SHAPE[1]))  # batch * 5 * 8
    ...
    def _intra_poi_self_attention(self, input):
        ...
        encoder_Q = tf.reshape(encoder_Q, (
        tf.shape(input)[0], FEATURE_CATE_NUM, CATE_FEATURE_EMBEDDINGS_SHAPE[1]))  # batch * 5 * 8
        encoder_K = tf.reshape(encoder_K, (
        tf.shape(input)[0], FEATURE_CATE_NUM, CATE_FEATURE_EMBEDDINGS_SHAPE[1]))  # batch * 5 * 8
        encoder_V = tf.reshape(encoder_V, (
        tf.shape(input)[0], FEATURE_CATE_NUM, CATE_FEATURE_EMBEDDINGS_SHAPE[1]))  # batch * 5 * 8
        ...
        output = tf.reshape(tf.matmul(attention_map, encoder_V),
                            (-1, FEATURE_CATE_NUM * CATE_FEATURE_EMBEDDINGS_SHAPE[1]))  # batch * 5 * 8
        return output
    ...
    def _build_model(self, features, labels, mode, params):
        ...
        with tf.name_scope('dnn_model'):
            ...
            fc_out = tf.reshape(self.poi_rep_emb, [-1, POI_NUM, FEATURE_CATE_NUM * CATE_FEATURE_EMBEDDINGS_SHAPE[
