...
class attention(tf.keras.layers.Layer):
    """隐藏层每一步的输出和候选item的注意力权重"""
    def __init__(self, embedding_size=EMBEDDING_SIZE, time_length=5, ):
        super().__init__()
        self.time_length = time_length
        self.embedding_size = embedding_size
        self.RepeatVector_time = tf.keras.layers.RepeatVector(self.time_length)  # tile
        ...
        self.Permute = tf.keras.layers.Permute((2, 1))  # reshape
    ...
    def call(self, inputs):
        ...
        repeated_candidate_layer = self.RepeatVector_time(candidate_inputs)  # b, e -> b, t, e
        activation_product_layer = self.Multiply([gru_hidden_state, repeated_candidate_layer])  # 点乘 b,t,e * b,t,e -> b,t,e
        activation_unit = self.Dense32(activation_product_layer)  # b, t, 32
        activation_unit = self.Dense1(activation_unit)  # b, t, 1
        Repeat_attention_s = tf.squeeze(activation_unit, axis=2)  # b, t  (每一步的权重)
        Repeat_attention_s = self.RepeatVector_emb(Repeat_attention_s)  # b, self.embedding_size, t
        Repeat_attention_s = self.Permute(Repeat_attention_s)  # b, t, self.embedding_size
...
class AUGRU(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        AUGRU_hidden_state = tf.reshape(initializer(shape=(1, self.embedding_size)), shape=(-1, self.embedding_size))  # -1, embedding_size
        for t in range(self.time_length):
            r_t = self.R_t([gru_hidden_state_inputs[:, t, :], AUGRU_hidden_state])  # [b, 1, embedding_size]
